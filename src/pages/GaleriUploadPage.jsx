...
import { galeriAPI } from '../utils/api';
...
export default function GaleriUploadPage() {
  ...
  // ── Load foto dari backend ─────────────────────────────────
  const loadFotos = async () => {
    setLoading(true);
    try {
      const result = await galeriAPI.getAll();
      if (result.success) setFotos(result.data || []);
    } catch { showToast('Gagal memuat foto dari server', 'err'); }
    setLoading(false);
  };
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!imgFile)              { showToast('Pilih gambar terlebih dahulu', 'err'); return; }
    if (!form.lokasi.length)   { showToast('Pilih minimal 1 lokasi tampil', 'err'); return; }

    setSaving(true);
    try {
      const result = await galeriAPI.upload(imgFile, {
        nama:    form.nama || 'Foto Kegiatan',
      ...
      if (result.success) {
        ...
        loadFotos(); // refresh dari backend
      } else {
        showToast(result.message || 'Upload gagal', 'err');
      }
  ...
  const handleToggleAktif = async (foto) => {
    try {
      const result = await galeriAPI.toggleAktif(foto.id);
      if (result.success) {
        setFotos(prev => prev.map(f => f.id===foto.id ? {...f, aktif: result.aktif} : f));
        if (preview?.id === foto.id) setPreview({...preview, aktif: result.aktif});
        showToast(result.aktif ? 'Foto ditampilkan.' : 'Foto disembunyikan.');
      }
    } catch { showToast('Gagal mengubah status', 'err'); }
  };

  // ── Update lokasi ──────────────────────────────────────────
  const handleUpdateLokasi = async (id, newLokasi) => {
    try {
      await galeriAPI.updateLokasi(id, newLokasi);
      setFotos(prev => prev.map(f => f.id===id ? {...f, lokasi: newLokasi} : f));
      if (preview?.id === id) setPreview({...preview, lokasi: newLokasi});
    } catch { showToast('Gagal update lokasi', 'err'); }
  };

  // ── Hapus foto ─────────────────────────────────────────────
  const handleDelete = async (id) => {
    try {
      const result = await galeriAPI.remove(id);
      if (result.success) {
      ...
      <main style={{ flex:1, padding:'32px 28px', overflowY:'auto' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24, flexWrap:'wrap', gap:12 }}>
          <div>
            ...
            <p style={{ fontSize:12, color:'#aaa' }}>Upload foto & atur lokasi tampil · Data dari server</p>
          </div>
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={loadFotos} style={{ padding:'11px 16px', background:'#f5f5f5', color:'#555', border:'1.5px solid #e0e0e0', borderRadius:14, fontFamily:'Poppins,sans-serif', fontSize:12, fontWeight:600, cursor:'pointer' }}>
              🔄 Refresh
            </button>
              ...
              <div style={{ background:'#fff', borderRadius:18, border:'1.5px solid rgba(192,57,43,0.2)', padding:'24px' }}>
                ...
                <form onSubmit={handleSave}>
                  ...
                  <div
                    ...
                    style={{ border: isDragging?'2.5px dashed #C0392B': imgPreview?'2px solid rgba(30,132,73,0.4)':'2px dashed #ddd', borderRadius:16, padding:'28px 20px', textAlign:'center', cursor:'pointer', marginBottom:20, background: isDragging?'rgba(192,57,43,0.04)': imgPreview?'rgba(30,132,73,0.03)':'#fafafa', transition:'all .2s' }}>
                    {imgPreview ? (
                      <div>
                        <div style={{ position:'relative', display:'inline-block', borderRadius:14, overflow:'hidden', marginBottom:12 }}>
                          <img src={imgPreview} alt="preview" style={{ maxHeight:200, maxWidth:'100%', borderRadius:14, display:'block' }}/>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:12, marginBottom:20 }}>
                    <div>
                      ...
                      <input type="text" placeholder="Deskripsi singkat foto..." value={form.nama} onChange={e=>setForm({...form,nama:e.target.value})}
                  ...
                  <div style={{ marginBottom:20 }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(190px,1fr))', gap:8 }}>
                      {LOKASI_OPTIONS.map(opt => {
                        ...
                        return (
                          <div key={opt.id} onClick={()=>toggleLokasi(opt.id)} style={{ border: sel?'2px solid #C0392B':'1.5px solid #f0f0f0', borderRadius:14, padding:'12px 14px', cursor:'pointer', background: sel?'rgba(192,57,43,0.05)':'#fafafa', transition:'all .2s', display:'flex', alignItems:'center', gap:10 }}>
                            <div style={{ width:30, height:30, borderRadius:'50%', flexShrink:0, background: sel?'#C0392B':'#f0f0f0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, transition:'all .2s' }}>
                              {sel ? '✓' : opt.icon}
                            </div>
                            <div>
                    ...
                    {!form.lokasi.length && <p style={{ fontSize:11, color:'#E74C3C', marginTop:8 }}>⚠️ Pilih minimal 1 lokasi tampil</p>}
                  </div>

                  <div style={{ display:'flex', gap:10 }}>
                    <button type="submit" disabled={saving} style={{ padding:'11px 26px', background: saving?'#ccc': (form.lokasi.length&&imgFile)?'#C0392B':'#ccc', color:'#fff', border:'none', borderRadius:12, fontFamily:'Poppins,sans-serif', fontSize:13, fontWeight:700, cursor: saving?'not-allowed':'pointer' }}>
                      {saving ? '⏳ Mengupload...' : '✅ Upload & Pasang Foto'}
                    </button>
                    <button type="button" onClick={()=>{ setShowForm(false); setImgFile(null); setImgPreview(null); }} style={{ padding:'11px 18px', background:'#f5f5f5', color:'#555', border:'none', borderRadius:12, fontFamily:'Poppins,sans-serif', fontSize:13, cursor:'pointer' }}>Batal</button>
                  </div>
                </form>
              </div>
            </motion.div>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:18, flexWrap:'wrap' }}>
          {programs.map(p=>(
            <button key={p} onClick={()=>setFilter(p)} style={{ padding:'6px 16px', borderRadius:20, border:'none', cursor:'pointer', fontFamily:'Poppins,sans-serif', fontSize:11, fontWeight:600, background: filter===p?'#C0392B':'#f5f5f5', color: filter===p?'#fff':'#666' }}>{p}</button>
          ))}
        </div>
        ...
        {loading ? (
          <div style={{ textAlign:'center', padding:'60px', color:'#aaa' }}>
            <div style={{ width:36, height:36, border:'3px solid #f0f0f0', borderTop:'3px solid #C0392B', borderRadius:'50%', animation:'spin .8s linear infinite', margin:'0 auto 12px' }}/>
            <p style={{ fontSize:13 }}>Memuat foto dari server...</p>
            <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign:'center', padding:'60px', color:'#aaa', fontSize:13 }}>
            <div style={{ fontSize:40, marginBottom:12 }}>🖼️</div>
            <p>Belum ada foto{filter!=='Semua'?' untuk kategori '+filter:''}. Upload foto baru!</p>
          </div>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))', gap:14 }}>
            {filtered.map((f,i)=>(
              <motion.div key={f.id} initial={{opacity:0,scale:0.93}} animate={{opacity:1,scale:1}} transition={{delay:i*0.04}}
                ...
                <div style={{ position:'relative', height:155 }}>
                  <img src={f.file_url} alt={f.nama} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }}
                    onError={e=>{ e.target.src='https://via.placeholder.com/300x155?text=Foto'; }}/>
                  ...
                  {!f.aktif && <div style={{ position:'absolute', top:8, right:8, background:'rgba(0,0,0,0.6)', color:'#fff', fontSize:9, padding:'2px 8px', borderRadius:10, fontWeight:600 }}>Tersembunyi</div>}
                ...
                <div style={{ padding:'10px 12px 12px' }}>
                  ...
                  <div style={{ fontSize:10, color:'#aaa' }}>📅 {new Date(f.created_at).toLocaleDateString('id-ID')} · 👤 {f.uploader_name}</div>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          {preview && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
              ...
              <motion.div initial={{scale:0.85,y:30}} animate={{scale:1,y:0}} exit={{scale:0.85}}
                ...
                <img src={preview.file_url} alt={preview.nama} style={{ width:'100%', height:220, objectFit:'cover', display:'block' }}
                  onError={e=>{ e.target.src='https://via.placeholder.com/480x220?text=Foto'; }}/>
                <div style={{ padding:'18px 20px' }}>
                  ...
                  <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:14 }}>
                    ...
                    <span style={{ fontSize:11, color:'#aaa' }}>👤 {preview.uploader_name}</span>
                  </div>
                          ...
                          return (
                            <div key={opt.id}
                              onClick={()=>{
                                ...
                                handleUpdateLokasi(preview.id, nl);
                              }}
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    <button onClick={()=>handleToggleAktif(preview)} style={{ flex:1, padding:'10px', border:'none', borderRadius:12, cursor:'pointer', fontFamily:'Poppins,sans-serif', fontSize:12, fontWeight:700, background: preview.aktif?'rgba(136,136,136,0.1)':'rgba(30,132,73,0.1)', color: preview.aktif?'#888':'#1E8449' }}>
                      {preview.aktif ? '👁️ Sembunyikan' : '✅ Tampilkan'}
                    </button>
                    <button onClick={()=>handleDelete(preview.id)} style={{ flex:1, padding:'10px', background:'rgba(192,57,43,0.1)', color:'#C0392B', border:'none', borderRadius:12, fontFamily:'Poppins,sans-serif', fontSize:12, fontWeight:700, cursor:'pointer' }}>🗑️ Hapus</button>
                    <button onClick={()=>{ setPreview(null); setEditLokasi(null); }} style={{ flex:1, padding:'10px', background:'#f5f5f5', color:'#555', border:'none', borderRadius:12, fontFamily:'Poppins,sans-serif', fontSize:12, cursor:'pointer' }}>Tutup</button>
                  </div>
                </div>
              </motion.div>
            </motion.div>
